...
export default function SettlementPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (serials.length > 0) {
        ...
        setTickets([]); // Clear tickets if none are pending
      }
    } catch (err) {
  ...
  const updateSelectionStatus = async (ticketSerial, matchName, newStatus) => {
    // FORCE LOWERCASE to satisfy database Check Constraints
    ...
    if (!error) {
      // If it's a loss, wait 500ms for the DB Trigger to delete other matches 
      // and update the 'print' table before we refresh the UI.
      if (statusLower === 'lost') {
  ...
  const finalizeTicket = async (ticketSerial, status) => {
    ...
    // REMOVED .toUpperCase() to match lowercase database constraint
    ...
    // Slight delay to ensure DB consistency before refresh
    setTimeout(() => {
      fetchData();
      setIsSettling(null);
    }, 300);
    ...
    <AdminLayout>
      <div className="p-8 bg-[#070a11] min-h-screen text-slate-200 font-sans">
        ...
        <div className="space-y-6">
                            ...
                            <div className="flex justify-between items-start mb-2">
                              <div className="flex flex-col max-w-[70%]">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="text-[8px] font-black text-white/40 uppercase tracking-widest">Matched Scraper</span>
          ...
          {!loading && tickets.length === 0 && (
            <div className="py-20 text-center font-black uppercase tracking-widest text-slate-700">No Pending Tickets Found</div>
          )}
        </div>
      </div>
    </AdminLayout>
