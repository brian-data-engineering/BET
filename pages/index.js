  ...
  const displayMatches = useMemo(() => {
    ...
    if (selectedLeague) {
      filtered = filtered.filter(m => 
        m.league_name === selectedLeague || m.display_league === selectedLeague
      );
    }
        ...
        <aside className="hidden lg:col-span-2 lg:block border-r border-white/5">
          <Sidebar 
            onSelectLeague={(league, sport) => {
                if (sport) setActiveTab(sport); // THE FIX: Automatically switch tab
        ...
        <main className="col-span-12 lg:col-span-7 bg-[#111926] min-h-screen border-r border-white/5">
          ...
          <div className="grid grid-cols-12 px-4 py-3 text-[10px] font-black text-slate-500 uppercase italic bg-[#0b0f1a]/30">
            <div className="col-span-7 flex items-center gap-2">
              Event Details 
              {selectedLeague && <span className="bg-[#10b981]/20 text-[#10b981] px-2 py-0.5 rounded border border-[#10b981]/30">{selectedLeague}</span>}
            </div>
          ...
          <div className="divide-y divide-white/5">
            {displayMatches.length > 0 ? (
                ...
                return (
                  <div key={match.id} className="grid grid-cols-12 p-3 items-center hover:bg-[#161f2e] transition-colors">
                    ...
                    <div className="col-span-5 grid grid-cols-3 gap-1.5 h-11">
                      {[{ label: '1', val: match.home_odds }, { label: 'X', val: match.draw_odds }, { label: '2', val: match.away_odds }].map((odd) => (
                        <button
                          key={odd.label}
                          ...
                          className={`rounded-md flex items-center justify-center font-black text-[10px] italic border transition-all ${
                            currentSelection?.selection === odd.label ? 'bg-[#10b981] border-[#10b981] text-white' : 'bg-[#1c2636] border-white/5 text-slate-300 hover:text-[#10b981]'
                          }`}
                        >
              ...
              <div className="py-20 text-center opacity-20">
                ...
                <button onClick={() => {setSelectedLeague(null); setSearchQuery('');}} className="mt-4 text-[9px] underline pointer-events-auto">Clear all filters</button>
              </div>
            )}
          </div>
        </main>
