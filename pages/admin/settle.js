...
import { Trophy, Search, Hash, Activity, CheckCircle2, XCircle, ChevronRight } from 'lucide-react';
...
export default function SettlementPage() {
  ...
  const fetchPendingTickets = async () => {
    ...
    try {
      ...
      const [ticketsRes, resultsRes, mappingsRes] = await Promise.all([
        ...
        supabase.from('soccer_results').select('*').order('created_at', { ascending: false }).limit(200),
      ...
      // 2. Sophisticated Matching Logic
      const ticketsWithData = (ticketsRes.data || []).map(ticket => {
        const enrichedSelections = (ticket.selections || []).map(sel => {
          ...
          // Get official names from mapping table
          ...
          // Find result by: ID -> Exact Mapped Names -> Partial Names (Botafogo)
  ...
  const checkLegStatus = (sel, res) => {
    ...
    // Market logic for 1X2, Over/Under, etc.
  ...
  const handleSettle = async (ticket, status) => {
    ...
    const { error } = await supabase.from('betsnow').update({ 
      status, 
      is_paid: status === 'won',
      settled_at: new Date() 
    }).eq('id', ticket.id);
    ...
    <AdminLayout>
      <div className="p-8 bg-[#070a11] min-h-screen text-slate-200">
        <header className="flex justify-between items-end mb-10">
          <div>
            <div className="flex items-center gap-2 mb-2 text-emerald-500">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em]">Lucra Settlement Console</span>
            </div>
        ...
        <div className="space-y-6">
          {loading ? (
            <div className="py-40 text-center font-black uppercase tracking-[1em] text-slate-800 text-xl">Aggregating Match Results...</div>
          ) : (
            tickets.filter(t => t.ticket_serial?.includes(searchTerm) || t.selections?.some(s => s.matchName?.toLowerCase().includes(searchTerm.toLowerCase()))).map(ticket => {
              ...
              return (
                <div key={ticket.id} className="bg-[#111926] border border-white/5 rounded-[2.5rem] p-8 flex flex-col lg:flex-row gap-8 items-stretch hover:border-white/10 transition-colors shadow-2xl">
                  {/* Left: Ticket Info */}
                  <div className="min-w-[220px] flex flex-col justify-between py-2 border-r border-white/5 pr-8">
                    <div>
                      <div className="text-[10px] font-black text-slate-600 mb-2 flex items-center gap-2 tracking-widest"><Hash size={12}/> {ticket.ticket_serial}</div>
                      <div className="text-4xl font-black text-emerald-500 italic leading-none mb-2">KES {ticket.potential_payout}</div>
                      <div className="text-[10px] font-bold text-slate-500 uppercase italic">Stake: {ticket.stake}</div>
                    </div>
                    <div className="text-[9px] font-black uppercase text-slate-500 bg-white/5 p-2 rounded-lg mt-4 inline-block text-center tracking-tighter">
                      CASHIER ID: {ticket.cashier_id?.substring(0,8) || 'SYSTEM'}
                    </div>
                  </div>

                  {/* Middle: Match Details with Raw Data Badges */}
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-2 mt-3">
                            ...
                            {s.res ? (
                              ...
                              <span className="text-[10px] bg-red-500/10 text-red-500 px-3 py-1.5 rounded-lg font-black border border-red-500/10 animate-pulse">MATCH DATA NOT FOUND</span>
                            )}
                          </div>
                        </div>
                  ...
                  <div className="min-w-[200px] flex flex-col gap-3 justify-center pl-4">
                    {isWon && <button onClick={() => handleSettle(ticket, 'won')} className="w-full bg-emerald-500 text-black py-5 rounded-2xl font-black uppercase italic text-xs hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all">APPROVE PAYOUT</button>}
                    {isLost && <button onClick={() => handleSettle(ticket, 'lost')} className="w-full bg-red-600/20 text-red-500 border border-red-500/30 py-5 rounded-2xl font-black uppercase italic text-xs hover:bg-red-500/30 transition-all">SETTLE LOSS</button>}
                    
                    <div className="pt-4 border-t border-white/5 mt-2">
                      <p className="text-[8px] font-black text-slate-700 uppercase tracking-widest text-center mb-3">Manual Override</p>
                      <div className="grid grid-cols-2 gap-2">
                        <button onClick={() => confirm("Force Win?") && handleSettle(ticket, 'won')} className="text-[9px] font-black bg-white/5 py-3 rounded-xl text-slate-600 hover:text-emerald-500 transition-all uppercase italic">Force Win</button>
                        <button onClick={() => confirm("Force Loss?") && handleSettle(ticket, 'lost')} className="text-[9px] font-black bg-white/5 py-3 rounded-xl text-slate-600 hover:text-red-500 transition-all uppercase italic">Force Loss</button>
                      </div>
                    </div>
                  </div>
                </div>
