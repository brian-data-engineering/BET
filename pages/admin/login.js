...
export default function AdminLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    if (profileError || !profile) {
      ...
      setErrorMsg("Profile sync error. Admin record not found.");
      setLoading(false);
      return;
    }
    ...
    if (profile.role === 'super_admin' || profile.role === 'admin') {
      ...
      // KICK THEM OUT: They have the password, but NOT the rank.
      await supabase.auth.signOut();
    ...
    <div className="min-h-screen bg-[#0b0f1a] text-white flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-md bg-[#111926] border border-white/5 rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
        ...
        <form onSubmit={handleLogin} className="relative z-10 space-y-6">
          ...
          <button 
            disabled={loading} 
            className="w-full bg-[#10b981] text-black font-black py-5 rounded-2xl hover:bg-white transition-all active:scale-[0.98] shadow-lg flex items-center justify-center gap-2 italic uppercase"
          >
            {loading ? (
