...
export default function ShopWallet() {
  ...
  const syncWalletData = useCallback(async () => {
    ...
    const [pRes, cRes, tRes] = await Promise.all([
      ...
      supabase.from('transactions').select('*, receiver:profiles!transactions_receiver_id_fkey(username)').eq('sender_id', user.id).order('created_at', { ascending: false }).limit(8)
    ]);
  ...
  const handleTransfer = async () => {
    ...
    // Safety check to prevent balance_not_negative error
    if (numAmount > (operatorProfile?.balance || 0)) {
      alert("Insufficient Float in Hub Vault.");
      return;
    }
    ...
    try {
      ...
      alert("Asset Dispatch Successful.");
    } catch (err) {
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-10 bg-[#0b0f1a] min-h-screen text-white font-sans">
        <div className="flex justify-between items-end border-b border-white/5 pb-10">
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.4em] mt-2 italic">Operator Node: {operatorProfile?.username}</p>
          </div>
          <div className="bg-[#111926] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic block mb-1">Available Vault Float</span>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <select className="w-full bg-[#0b0f1a] p-6 rounded-2xl border border-white/10 text-white font-bold outline-none appearance-none" value={targetId} onChange={e => setTargetId(e.target.value)}>
                  ...
                  {cashiers.map(c => <option key={c.id} value={c.id}>{c.username.toUpperCase()} (KES {parseFloat(c.balance).toLocaleString()})</option>)}
                </select>
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-500 uppercase ml-2 italic tracking-widest">Dispatch Amount (KES)</label>
                <input type="number" className="w-full bg-[#0b0f1a] p-8 rounded-2xl border border-white/10 text-5xl font-black text-[#10b981] outline-none" placeholder="0.00" value={amount} onChange={e => setAmount(e.target.value)} />
              </div>
            </div>
