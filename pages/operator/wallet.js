...
import { ArrowDownRight, Loader2, History, Clock } from 'lucide-react';
...
export default function ShopWallet() {
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      // DO NOT DO MATH HERE. Just reset the inputs.
      setAmount('');
      setTargetId('');

      // FORCE THE DATABASE TO TELL US THE NEW TRUTH
      await syncData(); 
      alert("Database Confirmed: Transfer Complete.");

    } catch (err) {
      alert("Critical Error: " + err.message);
      await syncData(); // Sync anyway to see what happened
    } finally {
    ...
    <OperatorLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-10 bg-[#0b0f1a] min-h-screen text-white font-sans">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className={`lg:col-span-7 bg-[#111926] p-12 rounded-[3rem] border border-white/5 space-y-10 shadow-2xl transition-opacity ${isProcessing ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <input type="number" className="w-full bg-[#0b0f1a] p-8 rounded-2xl border border-white/10 text-5xl font-black text-[#10b981] outline-none" placeholder="0" value={amount} onChange={e => setAmount(e.target.value)} />
              </div>
            </div>
            <button onClick={handleTransfer} disabled={isProcessing || !targetId || !amount} className="w-full bg-[#10b981] text-black font-black py-7 rounded-2xl hover:bg-white transition-all italic text-xs uppercase tracking-[0.3em] shadow-xl">
              {isProcessing ? 'Waiting for Database...' : 'Authorize Dispatch'}
            </button>
          </div>

          <div className="lg:col-span-5 bg-[#111926] p-10 rounded-[3rem] border border-white/5 shadow-2xl space-y-6">
            <h2 className="text-[10px] font-black uppercase text-slate-500 italic tracking-widest flex items-center gap-2">Audit Trail</h2>
              ...
              {transactions.map(tx => (
                <div key={tx.id} className="flex justify-between items-center p-5 bg-[#0b0f1a] rounded-2xl border border-white/5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                        <span className="text-xs font-black uppercase italic text-white/70 block">{tx.receiver?.username}</span>
