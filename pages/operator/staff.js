...
export default function ManageStaff() {
  ...
  const quickFund = async (id, name) => {
    ...
    if (error) alert("Transfer Failed: " + error.message);
      ...
      setTimeout(() => fetchStaffWithStats(operatorId), 300);
    }
  };
        ...
        <div className="flex justify-between items-end border-b border-white/5 pb-10">
          <div>
            <div className="flex items-center gap-2 text-blue-500 font-black uppercase text-[10px] italic tracking-widest mb-1">
              <RefreshCw size={12} className={fetching ? 'animate-spin' : ''} /> Network Infrastructure
            </div>
          ...
          <div className="bg-[#111926] px-10 py-6 rounded-[2.5rem] border border-white/5 shadow-2xl">
            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic block mb-1">Network Float</span>
              ...
              <tbody className="divide-y divide-white/5">
                {staff.map(s => (
                  <tr key={s.id} className="hover:bg-white/[0.02] transition-all">
                    ...
                    <td className="p-10 text-center font-black italic text-[#10b981]">{s.ticketCount} Tickets</td>
                    <td className="p-10 text-center font-black italic text-[#10b981]">KES {parseFloat(s.balance || 0).toLocaleString()}</td>
