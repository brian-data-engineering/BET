...
export default function ManageStaff() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (profile) {
        ...
        // 2. Fetch only AGENTS linked to this operator
  ...
  const handleCreateAgent = async (e) => {
    ...
    if (!operatorProfile?.tenant_id) {
      alert("Error: Operator profile not fully synchronized.");
      return;
    }
    ...
    try {
      ...
      alert(`AGENT PROVISIONED: ${cleanUsername} is active.`);
  ...
  const handleDispatch = async (id, name) => {
    ...
    const val = prompt(`Enter KES amount for ${name.toUpperCase()}:`);
    if (!val) return;
        ...
        {showAddForm && (
          <form onSubmit={handleCreateAgent} className="bg-[#111926] p-8 rounded-[2.5rem] border border-blue-500/20 flex flex-wrap gap-4 items-end animate-in fade-in zoom-in duration-200">
            <div className="flex-1 min-w-[200px]">
              ...
              <input required placeholder="agent_alpha" className="w-full bg-black/40 border border-white/5 p-4 rounded-2xl text-sm font-bold outline-none focus:border-blue-500 transition-all text-white" value={form.username} onChange={e => setForm({...form, username: e.target.value})} />
            </div>

            <div className="flex-1 min-w-[200px]">
              <label className="text-[10px] font-black uppercase text-slate-500 mb-2 block ml-2">Display Name</label>
              <input placeholder="City Center Hub" className="w-full bg-black/40 border border-white/5 p-4 rounded-2xl text-sm font-bold outline-none focus:border-blue-500 transition-all text-white" value={form.displayName} onChange={e => setForm({...form, displayName: e.target.value})} />
            </div>
            
            <div className="flex-1 min-w-[200px]">
              <label className="text-[10px] font-black uppercase text-slate-500 mb-2 block ml-2">Password</label>
              <input required type="password" placeholder="••••••••" className="w-full bg-black/40 border border-white/5 p-4 rounded-2xl text-sm font-bold outline-none focus:border-blue-500 transition-all text-white" value={form.password} onChange={e => setForm({...form, password: e.target.value})} />
            </div>
                  ...
                  <tr key={s.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="p-10">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-500 overflow-hidden">
                          {s.logo_url ? <img src={s.logo_url} className="w-full h-full object-cover" alt="" /> : <Monitor size={20} />}
                        </div>
                        <div>
