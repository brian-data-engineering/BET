...
import { Wallet, Users, TrendingUp, Activity, Loader2, Zap, Database } from 'lucide-react';
...
export default function OperatorDashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [ledgerRes, staffRes] = await Promise.all([
        // Fetch all ledger rows for the current operator
        supabase.from('ledger').select('type, amount').eq('user_id', user.id),
        // Count staff nodes
      ...
      // Calculate balance from ledger rows
      const computedBalance = (ledgerRes.data || []).reduce((acc, row) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // 2. THE NEW REALTIME: Listen to the LEDGER table, not profiles
      // Every time a transfer, bet, or payout hits the ledger, the dash refreshes
      channel = supabase
        .channel('dashboard-ledger-sync')
        .on('postgres_changes', 
          { 
            event: 'INSERT', 
            schema: 'public', 
          ...
          () => fetchDashboardData() // Re-sum the ledger on any new entry
        )
        .subscribe();
    };
      ...
      <div className="p-8 space-y-10 bg-[#0b0f1a] min-h-screen text-white font-sans">
        ...
        <div className="flex justify-between items-end border-b border-white/5 pb-8">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] italic">Ledger Verified</span>
            </div>
          ...
          <div className="bg-[#111926] p-4 rounded-2xl border border-white/5 flex items-center gap-3 px-6 shadow-2xl">
              ...
              <span className="text-[10px] font-black uppercase italic text-slate-400 tracking-widest">
                {loading ? "Calculating..." : "Ledger Live"}
              </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <StatCard 
            ...
            value={loading ? "..." : `KES ${stats.balance.toLocaleString()}`} 
            subtitle="Verified Vault Liquidity"
        ...
        <div className="relative group">
          <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/20 to-[#10b981]/20 rounded-[3rem] blur opacity-25 group-hover:opacity-50 transition duration-1000"></div>
          <div className="relative bg-[#111926] rounded-[3rem] border border-white/5 p-20 flex flex-col items-center justify-center text-center shadow-2xl overflow-hidden">
              ...
              {loading ? (
                ...
                <TrendingUp size={48} className="text-blue-500 mb-6 animate-pulse" />
              )}
              
              <h3 className="text-white font-black uppercase text-sm tracking-[0.5em] italic">
                {loading ? "Re-Summing Ledger..." : "Operational Integrity Verified"}
              </h3>
              <p className="text-[10px] text-slate-500 font-bold uppercase mt-4 italic tracking-widest max-w-sm leading-relaxed">
                All balances are computed from the immutable ledger. Manual profile overrides are now ignored by the system.
              </p>
          </div>
        </div>
...
function StatCard({ title, value, subtitle, icon, color, bg }) {
  return (
    <div className={`relative overflow-hidden bg-[#111926] p-10 rounded-[3rem] border ${color} shadow-2xl group transition-all hover:scale-[1.02]`}>
      <div className={`absolute top-0 right-0 w-40 h-40 ${bg} rounded-full -mr-20 -mt-20 blur-[80px] opacity-30`} />
      <div className="relative z-10 space-y-8">
        <div className="p-4 bg-[#0b0f1a] w-fit rounded-2xl border border-white/5">{icon}</div>
