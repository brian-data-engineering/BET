...
export default function OperatorSettings() {
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      // Sync selection limit to all profiles in this Operator's tenant
      ...
      alert("Failed to sync settings: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <OperatorLayout>
      <div className="p-8 max-w-3xl mx-auto space-y-10 bg-[#0b0f1a] min-h-screen text-white font-sans">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.4em] italic">Terminal Selection Policy</p>
          </div>
        ...
        <div className="bg-[#10b981]/5 border border-[#10b981]/10 p-6 rounded-3xl flex items-center gap-4">
          ...
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#10b981]/80">
            Note: This limit applies to all cashiers under your operator account.
          </p>
        </div>
