...
def fetch_deep_markets(session, parent_id):
    """Hits the individual match link using the 61... parent_match_id."""
    try:
...
def sync_lucra():
    ...
    # Keeping matches in DB for 3 hours after they start so they don't vanish during play
                ...
                for s_id, s_name in SPORTS.items():
                    ...
                    try:
                        ...
                        for match in matches:
                            # FIX: Use 'parent_match_id' (61...) as the primary link for markets
                            # Fallback to 'match_id' if parent is missing
                            ...
                            # Match Sync (Using the 61... ID as match_id)
                            cur.execute("""
