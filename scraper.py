...
# 1. Database Configuration - Reads from GitHub Secrets
DB_URL = os.getenv("DATABASE_URL")
MATCHES_URL = "https://api.betika.com/v1/uo/matches?limit=1000&sport_id=14"

if not DB_URL:
    print("❌ ERROR: DATABASE_URL not found. Check your GitHub Secrets.")
    sys.exit(1)
...
def sync_lucra():
    session = get_session()
    try:
        print(f"📡 [{datetime.now().strftime('%H:%M:%S')}] Connecting to API...")
        resp = session.get(MATCHES_URL, timeout=30)
        resp.raise_for_status()
        
        # Betika specific data structure
        json_data = resp.json().get("data", [])
        
        if not json_data:
            print("⚠️ No match data found in API response.")
            return
            ...
            with conn.cursor() as cur:
                for match in json_data:
                    try:
                        ...
                        p_id = str(match.get("parent_match_id")) if match.get("parent_match_id") else None
                        ...
                        # 3. Sync Match
                        cur.execute("""
                            INSERT INTO matches (match_id, game_id, competition_id, home_team, away_team, start_time, parent_match_id)
                            VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (match_id) 
                            DO UPDATE SET home_team=EXCLUDED.home_team, away_team=EXCLUDED.away_team, 
                                          start_time=EXCLUDED.start_time, updated_at=NOW()
                        """, (m_id, str(match.get("game_id")), c_id, match.get("home_team"), match.get("away_team"), match.get("start_time"), p_id))

                        # 4. Process NESTED Odds/Markets
                        for market in match.get("odds", []):
                            sub_id = str(market.get("sub_type_id"))
                            
                            # Update/Insert Markets
                            cur.execute("""
                            ...
                            for outcome in market.get("odds", []):
                                cur.execute("""
                                    INSERT INTO odds (match_id, sub_type_id, display, odd_key, odd_value, outcome_id)
                                    VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT (match_id, sub_type_id, odd_key)
                                    DO UPDATE SET odd_value=EXCLUDED.odd_value, outcome_id=EXCLUDED.outcome_id
                                """, (m_id, sub_id, outcome.get("display"), str(outcome.get("odd_key")), 
                                      float(outcome.get("odd_value") or 0), str(outcome.get("outcome_id"))))
        ...
        print(f"✨ [{datetime.now().strftime('%H:%M:%S')}] Supabase Sync Complete.")

    except Exception as e:
        print(f"❌ Critical Scraper Error: {e}")

if __name__ == "__main__":
    print("🚀 Lucra Cloud Engine v2.1")
    sync_lucra() # Run once then exit (GitHub Actions handles the timing)
