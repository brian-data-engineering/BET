          ...
          cart.map((item, idx) => (
            <div key={idx} className="bg-[#1c2636] rounded-2xl p-4 border border-white/5 relative group transition-all hover:border-[#10b981]/30">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase truncate pr-4">{item?.matchName || "Match"}</p>
              </div>
              <div className="flex justify-between items-end">
                <div className="space-y-1">
                  <p className="text-white font-black italic text-base uppercase tracking-tight">{item?.selection || "N/A"}</p>
