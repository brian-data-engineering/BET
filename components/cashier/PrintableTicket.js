...
export default function PrintableTicket({ ticket, cart = [], profiles = [], user }) {
  // 1. SAFE DATA UNIFICATION & PARSING
    ...
    <div className="print-only bg-white text-black w-[80mm] font-sans p-1 leading-tight">
        ...
        {selections.map((item, index) => (
          <div key={`${item.matchId}-${index}`} className="border-b border-black py-2 text-[11px]">
            ...
            <div className="flex items-start gap-2 mb-1">
              ...
              <span className="font-black uppercase text-[12px] leading-tight">{item?.matchName || "Match Name"}</span>
            </div>
            <div className="flex justify-between items-end mt-1">
              <div className="leading-none">
                <div className="text-[10px] font-bold text-gray-800">{item?.marketName || 'Match Result'}</div>
                <div className="text-[11px] font-black underline uppercase italic mt-1">Pick: {item?.selection || "N/A"}</div>
              </div>
              <div className="text-[16px] font-black">@{parseFloat(item?.odds || 0).toFixed(2)}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-2 space-y-1 font-bold text-[12px]">
        ...
        <div className="flex justify-between items-center bg-black text-white px-2 py-2 mt-1">
          ...
          <span className="text-[18px] font-black italic">{parseFloat(ticket?.potential_payout || 0).toLocaleString('en-KE')}</span>
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        @media print {
          .print-only { display: block !important; width: 80mm; background: white !important; margin: 0 auto; }
