  ...
  const fetchHierarchy = async () => {
    ...
    if (data) {
      ...
      data.forEach(item => {
        ...
        const country = league.includes('.') ? league.split('.')[0].trim() : 'International';

        // Skip virtual/esports leagues
        if (/(ebasketball|esoccer|srl|electronic|cyber)/i.test(league)) return;

        // FIX: cast tier_priority to Number — Supabase views can return strings
        const priority = Number(item.tier_priority);

        // Top Events — tier >= 500
        if (priority >= 500 && !seenLeagues.has(league)) {
          popular.push({ name: league, sport });
          seenLeagues.add(league);
        }

        // Build sport → country → leagues tree
          ...
          <div className="p-2 space-y-1">
            ...
            {topLeagues.map((league) => (
              ...
              >
                ...
                <ChevronRight size={12} className="text-slate-600 group-hover:text-[#10b981] shrink-0" />
              </button>
            ))}
                ...
                <div key={sportKey} className="rounded-xl overflow-hidden">
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3 transition-all ${
                      isSportExpanded
                        ? 'bg-[#10b981] text-[#0b0f1a] rounded-xl'
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {countryCount > 0 && (
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${isSportExpanded ? 'bg-[#0b0f1a]/20' : 'bg-white/10'}`}>
                          {countryCount}
                        </span>
                      )}
                      ...
                      {Object.keys(countries).sort().map(country => (
                        <div key={country} className="flex flex-col">
                          <button
                            ...
                            className="w-full px-4 py-2 flex items-center justify-between text-slate-500 hover:text-white transition-colors"
                          >
                            <span className="text-[10px] font-bold uppercase truncate">{country}</span>
                            <div className="flex items-center gap-1.5 shrink-0">
                              <span className="text-[8px] opacity-50">{countries[country].size}</span>
                          ...
                          {expandedCountry === country && (
                            <div className="bg-white/[0.02] ml-2 py-1">
                              {[...countries[country]].sort().map(league => (
                                <button
                                  key={league}
