  ...
  return (
    <div className="w-64 bg-[#0b0f1a] border-r border-white/5 flex flex-col h-screen sticky top-0 shadow-2xl">
      <div className="p-8 border-b border-white/5 flex flex-col gap-1">
        ...
        <p className="text-[8px] text-slate-600 font-black uppercase tracking-[0.4em] mt-1">Admin Interface v2.1</p>
      </div>

      <div className="mx-4 my-6 p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-3 shadow-inner">
        <div className="flex justify-between items-center text-[9px] font-black text-slate-500 uppercase italic tracking-widest">
        ...
        <div className="pt-2 border-t border-white/5 flex items-center gap-2 text-[8px] font-black text-slate-500 uppercase tracking-widest italic">
      ...
      <nav className="flex-1 px-4 space-y-1.5 overflow-y-auto custom-scrollbar">
        {isAuthorized ? adminMenu.map((item) => {
          const isActive = router.pathname === item.path;
          return (
            <Link key={item.name} href={item.path} className={`flex items-center gap-4 px-4 py-3.5 rounded-xl font-black text-[11px] uppercase italic tracking-widest transition-all group ${isActive ? 'bg-[#10b981] text-black shadow-[0_10px_25px_rgba(16,185,129,0.2)]' : 'text-slate-500 hover:bg-white/[0.03] hover:text-white'}`}>
              <span className={isActive ? 'text-black' : 'text-slate-600 group-hover:text-[#10b981] transition-colors'}>{item.icon}</span>
              {item.name}
            </Link>
          );
        }) : (
          <div className="p-4 text-[9px] font-black text-rose-500 uppercase italic bg-rose-500/5 rounded-xl border border-rose-500/10 text-center tracking-widest">Access Denied</div>
        )}
      </nav>
      ...
      <div className="p-4 border-t border-white/5 bg-[#080b13]">
        <div className="mb-4 px-4 py-3 rounded-xl bg-slate-900/40 border border-white/5 flex items-center gap-3">
           ...
           <div className="flex flex-col min-w-0 text-[8px] uppercase tracking-widest">
             <p className="font-black text-slate-600">Active Admin</p>
             <p className="font-black text-slate-300 truncate italic text-[11px]">{profile?.username || 'ROOT'}</p>
           </div>
        </div>
        <button onClick={handleSecureSignOut} className="flex items-center justify-center gap-3 w-full px-4 py-4 text-rose-500 font-black text-[10px] uppercase italic tracking-widest hover:bg-rose-500/10 rounded-xl transition-all border border-transparent hover:border-rose-500/20 active:scale-95">
